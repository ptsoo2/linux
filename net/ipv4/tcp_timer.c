...
static void tcp_probe_timer(struct sock* sk)
{
	...
	else if (icsk->icsk_user_timeout && (s32)(tcp_jiffies32 - icsk->icsk_probes_tstamp) >= msecs_to_jiffies(icsk->icsk_user_timeout))
		goto abort;
...
{
	struct inet_connection_sock* icsk = inet_csk(sk);
	int max_retries = icsk->icsk_syn_retries ?: sock_net(sk)->ipv4.sysctl_tcp_synack_retries + 1; /* add one more retry for fastopen */
	struct tcp_sock* tp = tcp_sk(sk);
	...
	inet_csk_reset_xmit_timer(sk, ICSK_TIME_RETRANS, TCP_TIMEOUT_INIT << req->num_timeout, TCP_RTO_MAX);
}
...
out_reset_timer:
	 ...
	 */
	if (sk->sk_state == TCP_ESTABLISHED && (tp->thin_lto || net->ipv4.sysctl_tcp_thin_linear_timeouts) && tcp_stream_is_thin(tp) && icsk->icsk_retransmits <= TCP_THIN_LINEAR_RETRIES)
	{
		icsk->icsk_backoff = 0;
...
void tcp_write_timer_handler(struct sock* sk)
{
	...
	if (((1 << sk->sk_state) & (TCPF_CLOSE | TCPF_LISTEN)) || icsk->icsk_pending == 0)
		goto out;
...
static void tcp_keepalive_timer(struct timer_list* t)
{
	...
	if (elapsed >= keepalive_time_when(tp))
	{
		 ...
		 */
		if ((icsk->icsk_user_timeout != 0 && elapsed >= msecs_to_jiffies(icsk->icsk_user_timeout) && icsk->icsk_probes_out > 0) || (icsk->icsk_user_timeout == 0 && icsk->icsk_probes_out >= keepalive_probes(tp)))
		{
			tcp_send_active_reset(sk, GFP_ATOMIC);
			tcp_write_err(sk);
			goto out;
