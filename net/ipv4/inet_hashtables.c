...
static void __inet_put_port(struct sock* sk)
{
	...
	const int bhash = inet_bhashfn(sock_net(sk), inet_sk(sk)->inet_num, hashinfo->bhash_size);
...
int __inet_inherit_port(const struct sock* sk, struct sock* child)
{
	...
	// 부모의 포트와 자식의 포트가 다른 경우
	if (tb->port != port)
	{
		l3mdev = inet_sk_bound_l3mdev(sk);

		/*
		 * NOTE: tproxy를 사용하고 스케줄러를 다른 리스너 포트로 리다이렉트하면 리스너 소켓의 icsk_bind_hash가 자식 소켓과 동일하다는 가정이 깨집니다.
		 * 여기에서 자식을 위해 새로운 바인드 버킷을 찾거나 만들어야 합니다.
		 */
		inet_bind_bucket_for_each(tb, &head->chain)
		{
			if (net_eq(ib_net(tb), sock_net(sk)) && tb->l3mdev == l3mdev && tb->port == port)
				break;
		}
		if (!tb)
		{
			tb = inet_bind_bucket_create(table->bind_bucket_cachep, sock_net(sk), head, port, l3mdev);
			if (!tb)
			{
				spin_unlock(&head->lock);
				return -ENOMEM;
...
static struct inet_listen_hashbucket* inet_lhash2_bucket_sk(struct inet_hashinfo* h, struct sock* sk)
{
	u32 hash;
...
#endif
		hash = ipv4_portaddr_hash(sock_net(sk), inet_sk(sk)->inet_rcv_saddr, inet_sk(sk)->inet_num);
...
static void inet_unhash2(struct inet_hashinfo* h, struct sock* sk)
{
	...
	if (!h->lhash2 || WARN_ON_ONCE(hlist_unhashed(&inet_csk(sk)->icsk_listen_portaddr_node)))
		return;
...
static inline int compute_score(struct sock* sk, struct net* net, const unsigned short hnum, const __be32 daddr, const int dif, const int sdif)
{
	int score = -1;
...
static inline struct sock* lookup_reuseport(struct net* net, struct sock* sk, struct sk_buff* skb, int doff, __be32 saddr, __be16 sport, __be32 daddr, unsigned short hnum)
{
...
/// @brief
///		여기에서 취약할 수 있는 몇 가지 좋은 속성이 있습니다.
// 		BSD API는 리스닝 소켓이 연결에 대한 원격 포트 또는 원격 주소를 지정하는 것을 허용하지 않습니다.
// 		따라서 검색 중에 항상 이들이 그렇지 않을 수 있기 때문에 이들이 모두 와일드카드로 설정되었다고 가정하십시오.
/// 		rcu_read_lock()으로 호출됨 : 소켓에 대한 참조 카운트가 증가하지 않음
/// @param net
/// @param ilb2
/// @param skb
/// @param doff
...
static inline struct sock* inet_lookup_run_bpf(struct net* net, struct inet_hashinfo* hashinfo, struct sk_buff* skb, int doff, __be32 saddr, __be16 sport, __be32 daddr, u16 hnum)
{
	...
	no_reuseport = bpf_sk_lookup_run_v4(net, IPPROTO_TCP, saddr, sport, daddr, hnum, &sk);
...
struct sock* __inet_lookup_established(struct net* net, struct inet_hashinfo* hashinfo, const __be32 saddr, const __be16 sport, const __be32 daddr, const u16 hnum, const int dif, const int sdif)
{
	...
	// 여기서 direct hit 을 위해 최적화합니다.
	// 어차피 리스닝 연결만 와일드카드를 가질 수 있습니다.

	// 두 엔드포인트와 프로토콜 해시를 조합한 해시를 계산합니다.
...
begin:
	...
	sk_nulls_for_each_rcu(sk, node, &head->chain)
	{
		...
		// 소켓이 일치하는지 확인합니다.
		if (INET_MATCH(sk, net, acookie, saddr, daddr, ports, dif, sdif) == true)
		{
			if (refcount_inc_not_zero(&sk->sk_refcnt) == false)
				goto out;

			// 한번 더 체크합니다.
			if (INET_MATCH(sk, net, acookie, saddr, daddr, ports, dif, sdif) == false)
			{
				sock_gen_put(sk);
				goto begin;
			}
	...
	// 이 검색의 끝에서 얻은 널 값이 예상 값과 다른 경우, 검색을 다시 시작해야 합니다.
	// 아마도 다른 체인으로 이동된 항목을 만났을 수 있습니다.
...
static int __inet_check_established(struct inet_timewait_death_row* death_row, struct sock* sk, __u16 lport, struct inet_timewait_sock** twp)
{
	...
	sk_nulls_for_each(sk2, node, &head->chain)
	{
		...
		if (likely(INET_MATCH(sk2, net, acookie, saddr, daddr, ports, dif, sdif)))
		{
 ...
 */
static bool inet_ehash_lookup_by_sk(struct sock* sk, struct hlist_nulls_head* list)
{
	...
	sk_nulls_for_each_rcu(esk, node, list)
	{
		...
		{
			if (unlikely(INET_MATCH(esk, net, acookie, sk->sk_daddr, sk->sk_rcv_saddr, ports, dif, sdif)))
			{
				return true;
			}
		}
		...
		{
			if (unlikely(INET6_MATCH(esk, net, &sk->sk_v6_daddr, &sk->sk_v6_rcv_saddr, ports, dif, sdif)))
			{
				return true;
			}
		}
...
/// @brief
///		소켓을 ehash에 삽입하고 다른 소켓(다른 소켓은 SYN_RECV 또는 TIMEWAIT 일 수 있음) 을 제거합니다.
///		이미 기존 소켓이 존재하는 경우, 소켓 sk가 삽입되지 않고 found_dup_sk 매개 변수가 true로 설정됩니다.
/// @param sk : 삽입할 소켓
/// @param osk : 제거할 소켓
/// @param found_dup_sk : 중복된 소켓이 발견되었는지 여부
...
static int inet_reuseport_add_sock(struct sock* sk, struct inet_listen_hashbucket* ilb)
{
	...
	{
		if (sk2 != sk && sk2->sk_family == sk->sk_family && ipv6_only_sock(sk2) == ipv6_only_sock(sk) && sk2->sk_bound_dev_if == sk->sk_bound_dev_if && inet_csk(sk2)->icsk_bind_hash == tb && sk2->sk_reuseport && uid_eq(uid, sock_i_uid(sk2)) && inet_rcv_saddr_equal(sk, sk2, false))
...
/// @brief
/// RFC 6056 3.3.4. 알고리즘 4:
/// 더블 해시 포트 선택 알고리즘. 2^16이 num_ephemeral의 배수가 아니기 때문에 32비트 정수를 사용합니다.
/// RFC는 TABLE_LENGTH=10 버킷을 사용하면 개선이 가능하다고 주장하지만, 우리는 256을 사용하여 더 많은 격리와 개인 정보 보호를 제공합니다.
/// 이는 커널 메모리 1KB만 소비합니다.
...
/// @brief
/// connect 작업을 위해 포트를 바인딩하고 해싱합니다.
/// @param death_row
/// @param sk
/// @return
int inet_hash_connect(struct inet_timewait_death_row* death_row, struct sock* sk)
{
	...
	// 로컬 포트가 지정되지 않은 경우 포트 할당 기준을 설정합니다.
	if (inet_sk(sk)->inet_num == 0)
...
void inet_hashinfo_init(struct inet_hashinfo* h)
{
	...
	for (i = 0; i < INET_LHTABLE_SIZE; i++)
	{
		...
		INIT_HLIST_NULLS_HEAD(&h->listening_hash[i].nulls_head, i + LISTENING_NULLS_BASE);
...
{
	h->lhash2 = alloc_large_system_hash(name, sizeof(*h->lhash2), numentries, scale, 0, NULL, &h->lhash2_mask, low_limit, high_limit);
