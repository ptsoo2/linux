...
{
	struct tcp_fastopen_context* ctx = container_of(head, struct tcp_fastopen_context, rcu);
...
void tcp_fastopen_destroy_cipher(struct sock* sk)
{
	...
	ctx = rcu_dereference_protected(inet_csk(sk)->icsk_accept_queue.fastopenq.ctx, 1);
	if (ctx)
...
static bool __tcp_fastopen_cookie_gen_cipher(struct request_sock* req, struct sk_buff* syn, const siphash_key_t* key, struct tcp_fastopen_cookie* foc)
{
	...
	if (req->rsk_ops->family == AF_INET)
	{
		...
		foc->val[0] = cpu_to_le64(siphash(&iph->saddr, sizeof(iph->saddr) + sizeof(iph->daddr), key));
	...
	if (req->rsk_ops->family == AF_INET6)
	{
		...
		foc->val[0] = cpu_to_le64(siphash(&ip6h->saddr, sizeof(ip6h->saddr) + sizeof(ip6h->daddr), key));
		foc->len = TCP_FASTOPEN_COOKIE_SIZE;
		return true;
	}
#endif
 ...
 */
static void tcp_fastopen_cookie_gen(struct sock* sk, struct request_sock* req, struct sk_buff* syn, struct tcp_fastopen_cookie* foc)
{
	struct tcp_fastopen_context* ctx;
...
static int tcp_fastopen_cookie_gen_check(struct sock* sk, struct request_sock* req, struct sk_buff* syn, struct tcp_fastopen_cookie* orig, struct tcp_fastopen_cookie* valid_foc)
{
...
static struct sock* tcp_fastopen_create_child(struct sock* sk, struct sk_buff* skb, struct request_sock* req)
{
	struct tcp_sock* tp;
	...
	child = inet_csk(sk)->icsk_af_ops->syn_recv_sock(sk, skb, req, NULL, NULL, &own_req);
	if (!child)
		return NULL;
	 ...
	 */
	inet_csk_reset_xmit_timer(child, ICSK_TIME_RETRANS, TCP_TIMEOUT_INIT, TCP_RTO_MAX);
...
static bool tcp_fastopen_no_cookie(const struct sock* sk, const struct dst_entry* dst, int flag)
{
	return (sock_net(sk)->ipv4.sysctl_tcp_fastopen & flag) || tcp_sk(sk)->fastopen_no_cookie || (dst && dst_metric(dst, RTAX_FASTOPEN_NO_COOKIE));
}
...
struct sock* tcp_try_fastopen(struct sock* sk, struct sk_buff* skb, struct request_sock* req, struct tcp_fastopen_cookie* foc, const struct dst_entry* dst)
{
	...
	if (!((tcp_fastopen & TFO_SERVER_ENABLE) && (syn_data || foc->len >= 0) && tcp_fastopen_queue_check(sk)))
	{
		foc->len = -1;
		return NULL;
	}
		...
		fastopen:
			...
			if (child)
			{
				if (ret == 2)
				{
					...
					*foc = valid_foc;
					NET_INC_STATS(sock_net(sk), LINUX_MIB_TCPFASTOPENPASSIVEALTKEY);
				}
				else
				{
					foc->len = -1;
...
bool tcp_fastopen_defer_connect(struct sock* sk, int* err)
{
	...
	if (tp->fastopen_connect && !tp->fastopen_req)
	{
		 ...
		 * in SYN
		 */
		tp->fastopen_req = kzalloc(sizeof(*tp->fastopen_req), sk->sk_allocation);
		if (tp->fastopen_req)
...
bool tcp_fastopen_active_should_disable(struct sock* sk)
{
	...
	timeout = READ_ONCE(sock_net(sk)->ipv4.tfo_active_disable_stamp) + multiplier * tfo_bh_timeout * HZ;
...
void tcp_fastopen_active_disable_ofo_check(struct sock* sk)
{
	...
	else if (tp->syn_fastopen_ch && atomic_read(&sock_net(sk)->ipv4.tfo_active_disable_times))
	{
		dst = sk_dst_get(sk);
...
void tcp_fastopen_active_detect_blackhole(struct sock* sk, bool expired)
{
	 ...
	 */
	if ((tp->syn_fastopen || tp->syn_data || tp->syn_data_acked) && (timeouts == 2 || (timeouts < 2 && expired)))
	{
		tcp_fastopen_active_disable(sk);
