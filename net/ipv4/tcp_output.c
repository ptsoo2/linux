...
/// @brief TCP 소켓의 시계를 새로고침하여 단조 증가하는 값을 보장합니다.
/// @param tp
...
static inline void tcp_event_ack_sent(struct sock* sk, unsigned int pkts, u32 rcv_nxt)
{
	...
	{
		NET_ADD_STATS(sock_net(sk), LINUX_MIB_TCPACKCOMPRESSED, tp->compressed_ack);
		tp->compressed_ack = 0;
...
static void tcp_ecn_send_syn(struct sock* sk, struct sk_buff* skb)
{
	...
	bool use_ecn = sock_net(sk)->ipv4.sysctl_tcp_ecn == 1 || tcp_ca_needs_ecn(sk) || bpf_needs_ecn;
	if (use_ecn == false)
	{
		const struct dst_entry* dst = __sk_dst_get(sk);
...
static void tcp_ecn_make_synack(const struct request_sock* req, struct tcphdr* th)
{
 ...
 * be sent.
 */
static void tcp_ecn_send(struct sock* sk, struct sk_buff* skb, struct tcphdr* th, int tcp_header_len)
{
	struct tcp_sock* tp = tcp_sk(sk);
...
/// @brief
/// 비 데이터 skb의 공통 제어 비트를 구성합니다.
/// SYN/FIN이 있으면 end seqno를 자동으로 증가시킵니다.
/// @param skb
/// @param seq
/// @param flags
...
static void smc_options_write(__be32* ptr, u16* options)
{
	...
	if (static_branch_unlikely(&tcp_have_smc))
	{
		...
		{
			*ptr++ = htonl((TCPOPT_NOP << 24) | (TCPOPT_NOP << 16) | (TCPOPT_EXP << 8) | (TCPOLEN_EXP_SMC_BASE));
...
static void mptcp_options_write(__be32* ptr, const struct tcp_sock* tp, struct tcp_out_options* opts)
{
...
static int bpf_skops_write_hdr_opt_arg0(struct sk_buff* skb, enum tcp_synack_type synack_type)
{
	if (unlikely(!skb))
...
static void bpf_skops_hdr_opt_len(struct sock* sk, struct sk_buff* skb, struct request_sock* req, struct sk_buff* syn_skb, enum tcp_synack_type synack_type, struct tcp_out_options* opts, unsigned int* remaining)
{
...
static void bpf_skops_write_hdr_opt(struct sock* sk, struct sk_buff* skb, struct request_sock* req, struct sk_buff* syn_skb, enum tcp_synack_type synack_type, struct tcp_out_options* opts)
{
...
static void tcp_options_write(__be32* ptr, struct tcp_sock* tp, struct tcp_out_options* opts)
{
	...
	{
		*ptr++ = htonl((TCPOPT_MSS << 24) | (TCPOLEN_MSS << 16) | opts->mss);
	}
	...
	if (likely(OPTION_TS & options))
	{
		...
		{
			*ptr++ = htonl((TCPOPT_NOP << 24) | (TCPOPT_NOP << 16) | (TCPOPT_TIMESTAMP << 8) | TCPOLEN_TIMESTAMP);
		}
	...
	{
		*ptr++ = htonl((TCPOPT_NOP << 24) | (TCPOPT_WINDOW << 16) | (TCPOLEN_WINDOW << 8) | opts->ws);
	}
	...
	if (unlikely(opts->num_sack_blocks))
	{
		...
		*ptr++ = htonl((TCPOPT_NOP << 24) | (TCPOPT_NOP << 16) | (TCPOPT_SACK << 8) | (TCPOLEN_SACK_BASE + (opts->num_sack_blocks * TCPOLEN_SACK_PERBLOCK)));
	...
	if (unlikely(OPTION_FAST_OPEN_COOKIE & options))
	{
		...
		if (foc->exp)
		{
			...
			*ptr = htonl((TCPOPT_EXP << 24) | (len << 16) | TCPOPT_FASTOPEN_MAGIC);
...
static void smc_set_option(const struct tcp_sock* tp, struct tcp_out_options* opts, unsigned int* remaining)
{
#if IS_ENABLED(CONFIG_SMC)
	if (static_branch_unlikely(&tcp_have_smc))
	{
...
static void smc_set_option_cond(const struct tcp_sock* tp, const struct inet_request_sock* ireq, struct tcp_out_options* opts, unsigned int* remaining)
{
#if IS_ENABLED(CONFIG_SMC)
	if (static_branch_unlikely(&tcp_have_smc))
	{
...
static void mptcp_set_option_cond(const struct request_sock* req, struct tcp_out_options* opts, unsigned int* remaining)
{
	if (rsk_is_mptcp(req))
	{
		unsigned int size;
...
static unsigned int tcp_synack_options(const struct sock* sk, struct request_sock* req, unsigned int mss, struct sk_buff* skb, struct tcp_out_options* opts, const struct tcp_md5sig_key* md5, struct tcp_fastopen_cookie* foc, enum tcp_synack_type synack_type, struct sk_buff* syn_skb)
{
 ...
 */
static unsigned int tcp_established_options(struct sock* sk, struct sk_buff* skb, struct tcp_out_options* opts, struct tcp_md5sig_key** md5)
{
	struct tcp_sock* tp = tcp_sk(sk);
	unsigned int size = 0;
	...
	*md5 = NULL;
	...
	if (sk_is_mptcp(sk))
	{
		...
		if (mptcp_established_options(sk, skb, &opt_size, remaining, &opts->mptcp))
		{
			opts->options |= OPTION_MPTCP;
			size += opt_size;
		}
	...
	if (unlikely(eff_sacks))
	{
		...
		if (unlikely(remaining < TCPOLEN_SACK_BASE_ALIGNED + TCPOLEN_SACK_PERBLOCK))
			return size;

		opts->num_sack_blocks = min_t(unsigned int, eff_sacks, (remaining - TCPOLEN_SACK_BASE_ALIGNED) / TCPOLEN_SACK_PERBLOCK);

		size += TCPOLEN_SACK_BASE_ALIGNED + opts->num_sack_blocks * TCPOLEN_SACK_PERBLOCK;
	}

	if (unlikely(BPF_SOCK_OPS_TEST_FLAG(tp, BPF_SOCK_OPS_WRITE_HDR_OPT_CB_FLAG)))
	{
		unsigned int remaining = MAX_TCP_OPTION_SPACE - size;
...
{
	if ((1 << sk->sk_state) & (TCPF_ESTABLISHED | TCPF_FIN_WAIT1 | TCPF_CLOSING | TCPF_CLOSE_WAIT | TCPF_LAST_ACK))
	{
		struct tcp_sock* tp = tcp_sk(sk);
...
/// @brief
/// 이 루틴은 실제로 tcp_do_sendmsg()에 의해 큐에 들어온 TCP 패킷을 전송합니다.
/// 이것은 초기 전송 및 나중에 재전송에 사용됩니다.
/// 여기서 본 모든 SKB는 완전히 헤더가 없습니다.
/// TCP 헤더를 작성하고 IP로 패킷을 전달하여 IP가 동일한 작업을 수행하고 패킷을 장치로 전달할 수 있도록해야합니다.
///
/// 여기서는 원래 SKB의 복제본 또는 재전송 엔진에 의해 만들어진 고유한 새 복사본 중 하나와 작업합니다.
/// @param sk
/// @param skb
/// @param clone_it
/// @param gfp_mask
...
static int __tcp_transmit_skb(struct sock* sk, struct sk_buff* skb, int clone_it, gfp_t gfp_mask, u32 rcv_nxt)
{
	...
	*(((__be16*)th) + 6) = htons(((tcp_header_size >> 2) << 12) | tcb->tcp_flags);
	...
	INDIRECT_CALL_INET(icsk->icsk_af_ops->send_check, tcp_v6_send_check, tcp_v4_send_check, sk, skb);
...
static void tcp_adjust_pcount(struct sock* sk, const struct sk_buff* skb, int decr)
{
	...
	if (tp->lost_skb_hint && before(TCP_SKB_CB(skb)->seq, TCP_SKB_CB(tp->lost_skb_hint)->seq) && (TCP_SKB_CB(skb)->sacked & TCPCB_SACKED_ACKED))
...
{
	return TCP_SKB_CB(skb)->txstamp_ack || (skb_shinfo(skb)->tx_flags & SKBTX_ANY_TSTAMP);
}
...
static void tcp_fragment_tstamp(struct sk_buff* skb, struct sk_buff* skb2)
{
	...
	if (unlikely(tcp_has_tx_tstamp(skb)) && !before(shinfo->tskey, TCP_SKB_CB(skb2)->seq))
	{
...
static void tcp_insert_write_queue_after(struct sk_buff* skb, struct sk_buff* buff, struct sock* sk, enum tcp_queue tcp_queue)
{
...
int tcp_fragment(struct sock* sk, enum tcp_queue tcp_queue, struct sk_buff* skb, u32 len, unsigned int mss_now, gfp_t gfp)
{
	...
	if (unlikely((sk->sk_wmem_queued >> 1) > limit && tcp_queue != TCP_FRAG_IN_WRITE_QUEUE && skb != tcp_rtx_queue_head(sk) && skb != tcp_rtx_queue_tail(sk)))
	{
...
int tcp_mss_to_mtu(struct sock* sk, int mss)
{
	...
	mtu = mss + tp->tcp_header_len + icsk->icsk_ext_hdr_len + icsk->icsk_af_ops->net_header_len;
...
/// @brief
/// MTU Probe, MSS 관련 값을 계산하여 캐싱합니다.
/// 이 함수는 snd mss를 현재 pmtu/exthdr 세트에 동기화합니다.
///
/// tp->rx_opt.user_mss는 TCP_MAXSEG에 의해 사용자가 설정한 mss입니다.
/// 이것은 TCP 옵션을 계산하지 않지만 순수한 TCP 헤더만 포함합니다.
///
/// tp->rx_opt.mss_clamp는 연결 설정 중에 협상된 mss입니다.
/// 이것은 user_mss와 SYN으로 수신된 mss의 최소값입니다.
/// 또한 TCP 옵션을 포함하지 않습니다.
///
/// inet_csk(sk)->icsk_pmtu_cookie는 이 함수에서 본 마지막 pmtu입니다.
///
/// tp->mss_cache는 SACK를 제외한 모든 tcp 옵션을 포함하는 현재 유효한 송신 mss입니다.
/// 현재 pmtu를 고려하여 평가되지만 tp->rx_opt.mss_clamp를 초과하지는 않습니다.
///
/// NOTE1. rfc1122는 명시적으로 광고된 MSS는 tcp 또는 ip 옵션을 포함하지 않는다고 명시합니다.
/// NOTE2. inet_csk(sk)->icsk_pmtu_cookie 및 tp->mss_cache는 이 함수 외부에서는 읽기 전용입니다.
/// --ANK (980731)
/// @param sk
/// @param pmtu
/// @return
...
static void tcp_cwnd_application_limited(struct sock* sk)
{
	...
	if (inet_csk(sk)->icsk_ca_state == TCP_CA_Open && sk->sk_socket && !test_bit(SOCK_NOSPACE, &sk->sk_socket->flags))
	{
...
static void tcp_cwnd_validate(struct sock* sk, bool is_cwnd_limited)
{
	 ...
	 */
	if (!before(tp->snd_una, tp->max_packets_seq) || tp->packets_out > tp->max_packets_out || is_cwnd_limited)
	{
	...
	else
	{
		 ...
		 */
		if (tcp_write_queue_empty(sk) && sk->sk_socket && test_bit(SOCK_NOSPACE, &sk->sk_socket->flags) && (1 << sk->sk_state) & (TCPF_ESTABLISHED | TCPF_CLOSE_WAIT))
 ...
 */
static void tcp_minshall_update(struct tcp_sock* tp, unsigned int mss_now, const struct sk_buff* skb)
{
...
static u32 tcp_tso_autosize(const struct sock* sk, unsigned int mss_now, int min_tso_segs)
{
	...
	bytes = min_t(unsigned long, sk->sk_pacing_rate >> READ_ONCE(sk->sk_pacing_shift), sk->sk_gso_max_size - 1 - MAX_TCP_HEADER);
...
static u32 tcp_tso_segs(struct sock* sk, unsigned int mss_now)
{
	...
	min_tso = ca_ops->min_tso_segs ? ca_ops->min_tso_segs(sk) : sock_net(sk)->ipv4.sysctl_tcp_min_tso_segs;
...
static unsigned int tcp_mss_split_point(const struct sock* sk, const struct sk_buff* skb, unsigned int mss_now, unsigned int max_segs, int nonagle)
{
	const struct tcp_sock* tp = tcp_sk(sk);
 ...
 */
static int tso_fragment(struct sock* sk, struct sk_buff* skb, unsigned int len, unsigned int mss_now, gfp_t gfp)
{
 ...
 */
static bool tcp_tso_should_defer(struct sock* sk, struct sk_buff* skb, bool* is_cwnd_limited, bool* is_rwnd_limited, u32 max_segs)
{
	const struct inet_connection_sock* icsk = inet_csk(sk);
...
static int tcp_mtu_probe(struct sock* sk)
{
	 ...
	 */
	if (likely(!icsk->icsk_mtup.enabled || icsk->icsk_mtup.probe_size || inet_csk(sk)->icsk_ca_state != TCP_CA_Open || tp->snd_cwnd < 11 || tp->rx_opt.num_sacks || tp->rx_opt.dsack))
		return -1;
...
static bool tcp_small_queue_check(struct sock* sk, const struct sk_buff* skb, unsigned int factor)
{
	...
	limit = max_t(unsigned long, 2 * skb->truesize, sk->sk_pacing_rate >> READ_ONCE(sk->sk_pacing_shift));
	if (sk->sk_pacing_status == SK_PACING_NONE)
...
bool tcp_schedule_loss_probe(struct sock* sk, bool advancing_rto)
{
	 ...
	 */
	if ((early_retrans != 3 && early_retrans != 4) || !tp->packets_out || !tcp_is_sack(tp) || (icsk->icsk_ca_state != TCP_CA_Open && icsk->icsk_ca_state != TCP_CA_CWR))
		return false;
...
void tcp_send_loss_probe(struct sock* sk)
{
	...
	if (unlikely(!skb))
	{
		WARN_ONCE(tp->packets_out, "invalid inflight: %u state %u cwnd %u mss %d\n", tp->packets_out, sk->sk_state, tp->snd_cwnd, mss);
	...
	{
		if (unlikely(tcp_fragment(sk, TCP_FRAG_IN_RTX_QUEUE, skb, (pcount - 1) * mss, mss, GFP_ATOMIC)))
			goto rearm_timer;
		skb = skb_rb_next(skb);
	}
...
void tcp_xmit_retransmit_queue(struct sock* sk)
{
	...
	if (rearm_timer)
		tcp_reset_xmit_timer(sk, ICSK_TIME_RETRANS, inet_csk(sk)->icsk_rto, TCP_RTO_MAX);
}
...
void sk_forced_mem_schedule(struct sock* sk, int size)
{
	...
	if (mem_cgroup_sockets_enabled && sk->sk_memcg)
		mem_cgroup_charge_skmem(sk->sk_memcg, amt, gfp_memcg_charge() | __GFP_NOFAIL);
}
...
struct sk_buff* tcp_make_synack(const struct sock* sk, struct dst_entry* dst, struct request_sock* req, struct tcp_fastopen_cookie* foc, enum tcp_synack_type synack_type, struct sk_buff* syn_skb)
{
	...
	tcp_header_size = tcp_synack_options(sk, req, mss, skb, &opts, md5, foc, synack_type, syn_skb) + sizeof(*th);
	...
	if (md5)
		tcp_rsk(req)->af_specific->calc_md5_hash(opts.hash_location, md5, req_to_sk(req), skb);
	rcu_read_unlock();
#endif
...
/// @brief
/// 혼잡 제어 알고리즘(ca, congestion algorithm) 모듈에서 목적지에 대한 초기화를 수행합니다.
/// @param sk
/// @param dst
...
static void tcp_connect_init(struct sock* sk)
{
	...
	// 다른 쪽에서 응답을 받을 때 이것을 수정할 것입니다.
	// tcp_input.c:tcp_rcv_state_process의 case TCP_SYN_SENT를 참조하십시오.
	{
	...
	// tp->mss_cache 를 계산하여 캐싱
	tcp_sync_mss(sk, dst_mtu(dst));

	// 혼잡 제어 알고리즘(ca, congestion algorithm) 모듈에서 목적지에 대한 초기화
	...
	tcp_select_initial_window(sk, tcp_full_space(sk), tp->advmss - (tp->rx_opt.ts_recent_stamp ? tp->tcp_header_len - sizeof(struct tcphdr) : 0), &tp->rcv_wnd, &tp->window_clamp, sock_net(sk)->ipv4.sysctl_tcp_window_scaling, &rcv_wscale, rcv_wnd);
...
/// @brief
/// 데이터와 (캐시된) Fast Open 쿠키가 있는 SYN을 빌드하고 전송합니다.
/// 그러나 일반적인 SYN 후에 데이터만 있는 패킷을 큐에 넣어 일반 SYN이 타임아웃될 때 재전송됩니다.
/// 또한 원격 SYN-ACK가 SYN 시퀀스만을 인정하는 경우 데이터가 첫 번째 ACK에서 재전송됩니다.
/// 만약 쿠키가 캐시되지 않았거나 다른 오류가 발생한다면, Fast Open 쿠키 요청 옵션을 포함한 일반 SYN을 보내도록 되돌아갑니다.
/// @param sk
/// @param syn
/// @return
static int tcp_send_syn_data(struct sock* sk, struct sk_buff* syn)
{
	...
	space = __tcp_mtu_to_mss(sk, inet_csk(sk)->icsk_pmtu_cookie) - MAX_TCP_OPTION_SPACE;
	...
	if (space)
	{
		int copied = copy_from_iter(skb_put(syn_data, space), space, &fo->data->msg_iter);
		if (unlikely(!copied))
		{
...
int tcp_connect(struct sock* sk)
{
	...
	if (inet_csk(sk)->icsk_af_ops->rebuild_header(sk))
		return -1 * EHOSTUNREACH; /* Routing failure or similar. */
	...
	struct sk_buff* buff = sk_stream_alloc_skb(sk, 0, sk->sk_allocation, true);
	if (buff == NULL)
		return -ENOBUFS;

	// 데이터가 포함되지 않은 skb의 공통 제어 비트를 구성합니다.
	...
	// SYN을 전송합니다; Fast Open에 데이터를 포함합니다.
	err = (tp->fastopen_req != NULL) //
			  ? tcp_send_syn_data(sk, buff)
			  : tcp_transmit_skb(sk, buff, 1, sk->sk_allocation);
	if (err == -1 * ECONNREFUSED)
		return err;
...
void tcp_send_delayed_ack(struct sock* sk)
{
	...
	if (ato > TCP_DELACK_MIN)
	{
		...
		if (inet_csk_in_pingpong_mode(sk) || (icsk->icsk_ack.pending & ICSK_ACK_PUSHED))
...
/// @brief
/// 이 루틴은 ack를 보내고, 또한 window 를 갱신합니다.
/// @param sk
/// @param rcv_nxt
void __tcp_send_ack(struct sock* sk, u32 rcv_nxt)
{
	...
	// 이것을 쓰기 큐에 넣지 않기 때문에 tcp_transmit_skb()는 이 소켓에 소유권을 설정합니다.
...
int tcp_rtx_synack(const struct sock* sk, struct request_sock* req)
{
	...
	// res = tcp_v4_send_synack(sk, NULL, &fl, req, NULL, TCP_SYNACK_NORMAL, NULL);
	if (res == 0)
	{
