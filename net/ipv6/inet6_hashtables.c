...
u32 inet6_ehashfn(const struct net* net, const struct in6_addr* laddr, const u16 lport, const struct in6_addr* faddr, const __be16 fport)
{
	...
	return __inet6_ehashfn(lhash, lport, fhash, fport, inet6_ehash_secret + net_hash_mix(net));
}
...
static inline int compute_score(struct sock* sk, struct net* net, const unsigned short hnum, const struct in6_addr* daddr, const int dif, const int sdif)
{
	int score = -1;

	if (net_eq(sock_net(sk), net) && inet_sk(sk)->inet_num == hnum && sk->sk_family == PF_INET6)
	{
...
static inline struct sock* lookup_reuseport(struct net* net, struct sock* sk, struct sk_buff* skb, int doff, const struct in6_addr* saddr, __be16 sport, const struct in6_addr* daddr, unsigned short hnum)
{
...
static inline struct sock* inet6_lookup_run_bpf(struct net* net, struct inet_hashinfo* hashinfo, struct sk_buff* skb, int doff, const struct in6_addr* saddr, const __be16 sport, const struct in6_addr* daddr, const u16 hnum)
{
	...
	no_reuseport = bpf_sk_lookup_run_v6(net, IPPROTO_TCP, saddr, sport, daddr, hnum, &sk);
...
struct sock* inet6_lookup_listener(struct net* net, struct inet_hashinfo* hashinfo, struct sk_buff* skb, int doff, const struct in6_addr* saddr, const __be16 sport, const struct in6_addr* daddr, const unsigned short hnum, const int dif, const int sdif)
{
	...
	{
		result = inet6_lookup_run_bpf(net, hashinfo, skb, doff, saddr, sport, daddr, hnum);
		if (result)
			goto done;
	}
...
struct sock* inet6_lookup(struct net* net, struct inet_hashinfo* hashinfo, struct sk_buff* skb, int doff, const struct in6_addr* saddr, const __be16 sport, const struct in6_addr* daddr, const __be16 dport, const int dif)
{
	...
	sk = __inet6_lookup(net, hashinfo, skb, doff, saddr, sport, daddr, ntohs(dport), dif, 0, &refcounted);
...
static int __inet6_check_established(struct inet_timewait_death_row* death_row, struct sock* sk, const __u16 lport, struct inet_timewait_sock** twp)
{
	...
	const unsigned int hash = inet6_ehashfn(net, daddr, lport, saddr, inet->inet_dport);
...
static u32 inet6_sk_port_offset(const struct sock* sk)
{
	...
	return secure_ipv6_port_ephemeral(sk->sk_v6_rcv_saddr.s6_addr32, sk->sk_v6_daddr.s6_addr32, inet->inet_dport);
}
...
int inet6_hash_connect(struct inet_timewait_death_row* death_row, struct sock* sk)
{
	...
	return __inet_hash_connect(death_row, sk, port_offset, __inet6_check_established);
}
