...
struct request_sock_ops
{
	...
	char* slab_name;
	int (*rtx_syn_ack)(const struct sock* sk, struct request_sock* req);
	void (*send_ack)(const struct sock* sk, struct sk_buff* skb, struct request_sock* req);
	void (*send_reset)(const struct sock* sk, struct sk_buff* skb);
...
{
	struct request_sock* req = kmem_cache_alloc(ops->slab, GFP_ATOMIC | __GFP_NOWARN);
	if (req == NULL)
		return NULL;

	req->rsk_listener = NULL;
	if (attach_listener != NULL)
	{
